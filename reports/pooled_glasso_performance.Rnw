...
\title{Sparse Undirected Graphs for Spatiotemporal Modeling: Summary of Pooled GLASSO Performance}
...
<<glasso>>=
glasso_results <- Pooled_Glasso(DF, var_list,
...
<<calculate_glasso_error, results='none'>>=
error_curve <- plot_error_curve(glasso_results)
@
<<show_glasso_error, fig.cap=error_curve$plot_caption>>=
print(error_curve$out_plot)
@
...
p1 <- plot_spatial_correlation(glasso_results)
